...
const SINGLELOAD = "dictionary/SINGLELOAD";
...
export function singleLoadDictionary(singleDictionary) {
  console.log("하나만 불러올거야!");
  return { type: SINGLELOAD, singleDictionary };
}
...
export const updateDictionaryFB = (dictionary_id, dics) => {
...
export const singleLoadDictionaryFB = (editId) => {
  return async function (dispatch) {
    console.log("singleId : " + editId)
    const docRef = doc(db, "dictionary", editId);
    const data = await getDoc(docRef);
    const tempData = { id: data.id, ...data.data() }

    console.log("singleLoadDictionaryFB : " + data)
  
   dispatch(singleLoadDictionary(tempData));
  };
};
...
export default function reducer(state = initialState, action = {}) {
  ...
  switch (action.type) {
    ...
    case "dictionary/SINGLELOAD": {
      return {singleData: action.singleDictionary, is_loaded: true };
    }
